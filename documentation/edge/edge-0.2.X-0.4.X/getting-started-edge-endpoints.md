...
- [Forwarding Data](getting-started-edge-endpoints#forwarding-data1)
    - *Endpoint Types*
...
Once you have [connected to the Edge and configured network settings](getting-started-connect) you can manage sending data to your specified endpoints on the Edge.
...
Under BlueCats > Forwarding > Filters, you can add filters to your data. When BLE advertisements are received from each scan they can be filtered using one or more rules.

1. Use RSSI threshold: Only ads with a minimum received signal strength indicator (RSSI). [This is under the Forwarding parameters](getting-started-edge-endpoints#forwarding-parameters). 
...
### Example - MQTT
Once the Edge has been configured to send messages over MQTT to a port and IP, we can set up a simple MQTT server application to listen for the messages.

Here is an example in a third party Broker MQTT client called [HiveMQ](http://www.hivemq.com/demos/websocket-client/)
...
To receive the data we will need to set up a local HTTP server to listen on the port configured for the HTTP endpoint. Here is an example in Python:
