...
*Basic Authorization Credentials* is returned after a GET request for
user login. Adding the *Basic Authorization Credentials* to your header
allows you to make requests POST/GET groups.
...
Performing a GET request on *groups* allows you to see all of the
*groups* that you are authorized to access. Querying groups also allows
you to determine *Groups IDs*. *Group IDs* allow you to create objects
in that groups and perform other meaningful queries.
...
Performing a GET request on *objectTypes* is useful to determine which
objects are available to which groups and which you can query. The group
ID in the objectTypes shows what group created and has ownership of the
objectType. Anyone underneath that group can create objects. When
creating objects, check which object keys are required and what types
they are when you want to create objects.
...
+---------------------+------------------------------------------------+
| *body*              | { “groupID”: {groupID}, “requiredKey”:         |
|                     | {requiredValue}, “optionalKey”:                |
|                     | {optionalValue} }                              |
+---------------------+------------------------------------------------+
...
+=================+============+===========+===============+==========+
| objectType      | *string*   | Yes       | specifying    | objectTy |
|                 |            |           | objectType to | pe=bcdPe |
|                 |            |           | query         | rson     |
+-----------------+------------+-----------+---------------+----------+
| limit           | *int*      | No        | Number of     | limit=34 |
|                 |            |           | objects       |          |
|                 |            |           | returned (max |          |
|                 |            |           | 200)          |          |
+-----------------+------------+-----------+---------------+----------+
| first           | *int*      | No        | Index of      | first=67 |
|                 |            |           | object to     |          |
|                 |            |           | start from    |          |
|                 |            |           | (when paging) |          |
+-----------------+------------+-----------+---------------+----------+
| sortkey         | *string*   | No        | Sorting       | sortkey= |
|                 |            |           | objects by a  | name     |
|                 |            |           | key (defaults |          |
|                 |            |           | to ascending) |          |
+-----------------+------------+-----------+---------------+----------+
| sortdir         | *string*   | No        | Sorting       | sortkey= |
|                 |            |           | objects by a  | name&sor |
|                 |            |           | key and       | tdir=des |
|                 |            |           | direction     | c        |
|                 |            |           | ascending or  |          |
|                 |            |           | descending    |          |
|                 |            |           | (asc or desc) |          |
+-----------------+------------+-----------+---------------+----------+
| ssearch         | *string*   | No        | Index of      | ssearch= |
|                 |            |           | object to     | ernie    |
|                 |            |           | start from,   |          |
|                 |            |           | when paging   |          |
+-----------------+------------+-----------+---------------+----------+
